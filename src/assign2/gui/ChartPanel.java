...
/**
 * ChartPanel A class extends the JPanel, to create and control the chart area
 * 
 * @version 1.0
...
public class ChartPanel extends JPanel {
	...
	private JFreeChart createChart(CategoryDataset dataset, String ChartTitle) {
		...
		JFreeChart chart = ChartFactory.createBarChart3D(ChartTitle, "Phrase",
	...
	private DefaultCategoryDataset createDataset(NGramContainer ngram) {
		...
		for (int count = 0; count < ngram.getPredictions().length; count++) {
			...
			dataset.setValue(ngram.getProbabilities()[count],
					ngram.getContext(), ngram.getPredictions()[count]);
		}
		return dataset;
	}
	...
	// createDataset**********************************************
	/**
	 * clearResultsChart For clearing the result Chart
	 * 
	 */
